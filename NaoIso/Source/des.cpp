...
//-------------------------------------------------------------------------
//
// This is the source from GnuPG for the DES Encryption/Decryption
// routines. We use it here to encrypt the FTP passwords
//
//-------------------------------------------------------------------------
//
// $Log: des.c,v $
// Revision 1.1  2000/11/12 16:33:45  fraggle
// Add DES Encryption for passwords
//
//
//-------------------------------------------------------------------------
  ...
  while(left <= right)
    {
      ...
      if ( !(cmp_result=working_memcmp((const char *)work, (const char *) weak_keys[middle], 8)) )
	  return -1;
...
{
    if( selftest_failed )
      return -1;                 //G10ERR_SELFTEST_FAILED;
    if( keylen != 24 )
      return -1;                 //G10ERR_WRONG_KEYLEN;

    tripledes_set3keys ( ctx, key, key+8, key+16);

    if( is_weak_key( key ) || is_weak_key( key+8 ) || is_weak_key( key+16 ) )
      return -1;           //G10ERR_WEAK_KEY;
